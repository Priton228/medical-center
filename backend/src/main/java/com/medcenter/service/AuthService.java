...
public class AuthService {
    ...
    public AuthDtos.JwtResponse register(AuthDtos.RegisterRequest req) {
        ...
        if (req.role() == RoleName.ROLE_ADMIN) {
            throw new BadRequestException("Регистрация администратора запрещена");
        }
        ...
        if (req.role() == RoleName.ROLE_PATIENT) {
        ...
        } else if (req.role() == RoleName.ROLE_DOCTOR) {
            String spec = req.specialization() != null && !req.specialization().isBlank()
                ? req.specialization() : "Терапевт";
